...
- Measured A/B at 1M rows: sorts do the same or better wall time with **4.4× less main-thread work** (300ms vs 1,325ms); filters comparable with a one-time ~0.2s text-projection handoff on first use; deltas are main-thread either way.
- A third mode, `hybrid` (filter main-thread, sort worker fed by the filter's candidate indices), measured as the best all-rounder — main-thread filter latency, worker sort CPU profile, ~0.3MB worker heap for filtering vs ~99MB full-worker — and became the **default** in v0.6.0. Main-thread remains the no-Worker fallback.
