...
| `workerHeapMB` | The pipeline worker's heap at scenario end (V8 heap + typed-array backing stores, where projections live). Dedicated workers expose no JS memory API, so the bench runner reads it over CDP (`Runtime.getHeapUsage` on the worker target); demo pages in headed Chrome attribute it via `measureUserAgentSpecificMemory` (the app serves COOP/COEP for this). Absent on the main-thread executor. Total memory in worker/hybrid modes ≈ `cdpJsHeapUsedMB` + `workerHeapMB`. |
